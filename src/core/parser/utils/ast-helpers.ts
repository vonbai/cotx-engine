...
const rustOwnerLabelCache = new WeakMap<SyntaxNode, Map<string, NodeLabel>>();

function rustImplTypeName(node: SyntaxNode): string {
  const text = node.text;
  const lastScopeSegment = text.includes('::') ? text.slice(text.lastIndexOf('::') + 2) : text;
  const genericStart = lastScopeSegment.indexOf('<');
  return genericStart >= 0 ? lastScopeSegment.slice(0, genericStart) : lastScopeSegment;
}

function rustTopLevelTypeLabels(root: SyntaxNode): Map<string, NodeLabel> {
  const cached = rustOwnerLabelCache.get(root);
    ...
    if (node.type === 'struct_item' || node.type === 'enum_item' || node.type === 'trait_item') {
      ...
      if (nameNode) {
        const label: NodeLabel =
          node.type === 'enum_item' ? 'Enum' : node.type === 'trait_item' ? 'Trait' : 'Struct';
        result.set(nameNode.text, label);
      }
      return;
    }
  ...
  visit(root, 0);
  rustOwnerLabelCache.set(root, result);
  return result;
}
    ...
    if (CLASS_CONTAINER_TYPES.has(current.type)) {
      ...
      if (current.type === 'impl_item') {
        ...
        if (forIdx !== -1) {
          ...
          if (nameNode) {
            const className = rustImplTypeName(nameNode);
            const label = rustTopLevelTypeLabels(current.tree.rootNode).get(className) ?? 'Struct';
            return {
              classId: generateId(label, `${filePath}:${className}`),
              className,
            };
          }
        }
        ...
        if (firstType) {
          const className = rustImplTypeName(firstType);
          const label = rustTopLevelTypeLabels(current.tree.rootNode).get(className) ?? 'Impl';
          return {
            classId: generateId(label, `${filePath}:${className}`),
            className,
          };
