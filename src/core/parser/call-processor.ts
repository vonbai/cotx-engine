...
export const buildImplementorMap = (
  ...
  // Phase D step 7 follow-up: sort for determinism (warm vs cold extract order).
...
const resolveCallTarget = (
  ...
  if (call.callForm === 'member' && call.receiverTypeName) {
    ...
    if (typeResolved && typeResolved.candidates.length > 0) {
      ...
      let methodPool: ReadonlyArray<(typeof filteredCandidates)[number]>;
      if (filteredCandidates.length <= 1) {
        ...
        if (cached) {
          ...
          methodPool = filterCallableCandidates(
...
export const processCallsFromExtracted = async (
  ...
  // Phase D step 7 follow-up: sort files + calls deterministically so warm
  // runs with different insertion order (cache hits + fresh misses vs all
  // fresh) produce byte-identical output. Without this, downstream edge
  // generation order drifts between runs and breaks projection-hash
  // short-circuits.
  const byFile = new Map<string, ExtractedCall[]>();
  const sortedCalls = [...extractedCalls].sort((a, b) => {
    const cmp = a.filePath.localeCompare(b.filePath);
    if (cmp !== 0) return cmp;
    const ids = a.sourceId.localeCompare(b.sourceId);
    if (ids !== 0) return ids;
    return (a.calledName ?? '').localeCompare(b.calledName ?? '');
  });
