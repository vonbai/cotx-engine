    ...
    details = await Promise.race([
      Promise.resolve(
        leiden.detailed(graph, {
          ...
          // Note: maxIterations is NOT accepted by vendor/leiden/index.cjs
          // (not in its DEFAULTS). Silently ignored if passed. Removed to avoid
          // confusion; Leiden runs until modularity convergence.
          rng: seedrandom('cotx-deterministic'),
        }),
      ),
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => reject(new Error('Leiden timeout')), LEIDEN_TIMEOUT_MS);
        timeout.unref?.();
      }),
    ]);
  } catch (e: any) {
...
const buildGraphologyGraph = (knowledgeGraph: KnowledgeGraph, isLarge: boolean): GraphInstance => {
  ...
  // Phase D step 8: collect relationships into a sorted array before any
  // graph mutation. Leiden is deterministic given the seeded RNG AND stable
  // graph iteration order. Graphology preserves insertion order in Maps, so
  // sorting the insertion sequence fixes determinism across runs where
  // upstream parsing may hand nodes/edges to us in different orders.
  ...
  sortedRels.sort((a, b) => {
    ...
    return cmp !== 0 ? cmp : a.targetId.localeCompare(b.targetId);
  });
  ...
  const sortedNodes: NodeSnapshot[] = [];
  knowledgeGraph.forEachNode((node) => {
