import type { GraphNode, GraphRelationship, NodeLabel } from '../shared/index.js';
...
export interface WorkerExtractedData {
  ...
  /**
   * Phase D step 2: raw per-worker ParseWorkerResult. Populated only when
   * the caller requests it (via captureRawResults), so the parse cache can
   * split by file and persist. Undefined in sequential fallback.
   */
  rawChunkResults?: ParseWorkerResult[];
}
...
const processParsingWithWorkers = async (
  ...
  for (const result of chunkResults) {
    ...
    extendArray(allImports, result.imports);
    extendArray(allCalls, result.calls);
    extendArray(allAssignments, result.assignments);
    extendArray(allHeritage, result.heritage);
    extendArray(allRoutes, result.routes);
    extendArray(allFetchCalls, result.fetchCalls);
    extendArray(allDecoratorRoutes, result.decoratorRoutes);
    extendArray(allToolDefs, result.toolDefs);
    if (result.ormQueries) extendArray(allORMQueries, result.ormQueries);
    extendArray(allConstructorBindings, result.constructorBindings);
    extendArray(allTypeEnvBindings, result.typeEnvBindings);
  }
  ...
  return {
    ...
    rawChunkResults: captureRawResults ? chunkResults : undefined,
  };
};
