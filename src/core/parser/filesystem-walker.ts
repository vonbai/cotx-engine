...
export const walkRepositoryPaths = async (
  ...
  // Transparency: flag top-level directories contributing the most files.
  // Users frequently discover they're scanning `example/` or `corpus_*` they
  // forgot about. We don't auto-exclude (too risky) — just emit a hint.
  ...
  const large = [...topDirCounts.entries()]
    .filter(([, count]) => count >= LARGE_DIR_THRESHOLD)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10);
  if (large.length >= 3 && entries.length > 2000) {
    const summary = large.map(([dir, count]) => `${dir}=${count}`).join(', ');
    console.warn(
      `  Scan breakdown (top-level dirs, files≥${LARGE_DIR_THRESHOLD}): ${summary}. ` +
        `Add any unwanted dir to .cotxignore to skip it.`,
    );
  }
