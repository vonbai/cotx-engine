...
import { extendArray } from '../shared/array-utils.js';
...
import {
  ...
  persistParsedFiles,
  hashFileContent,
  buildGlobalNodeFileMap,
  type PerFileParseData,
} from './parse-cache-integration.js';
import {
  computeProcessInputsHash,
  loadCachedProcesses,
  saveCachedProcesses,
...
export interface PipelineOptions {
  ...
  /**
   * Phase D step 2: if provided, the parse phase consults this cache on a
   * per-file content-hash basis. Cache hits skip Tree-sitter entirely and
   * get re-injected into the live graph. Cache misses are parsed normally
   * and then persisted back. Passing `undefined` disables parse caching
   * (legacy behavior). Honored only when COTX_FORCE_FULL is not set.
   */
  incrementalCache?: import('../../compiler/incremental-cache.js').IncrementalCache;
}
...
async function runChunkedParseAndResolve(
  ...
  try {
    // Phase D step 2: cache split. Both writes and reads default-on when
    // a cache is provided (unless COTX_FORCE_FULL=1). The subprocess retry
    // wrapper (Phase D step 2c, `commands/compile-retry-wrapper.ts`) catches
    // the intermittent native tree-sitter Napi abort and retries with
    // force-full, so end users never see the crash.
    const cacheActive =
      options?.incrementalCache !== undefined && process.env.COTX_FORCE_FULL !== '1';
    const cacheWritesActive = cacheActive;
    const cacheReadsActive = cacheActive;
    const cache = cacheActive ? options!.incrementalCache! : null;
    let cacheHitFiles = 0;
    ...
    for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
      ...
      // Cache split. Always compute content hashes so writes can persist,
      // but only actually skip parsing for HITS when COTX_PARSE_CACHE=1.
      let filesToParse = chunkFiles;
      let cachedHits: PerFileParseData[] = [];
      const contentHashByPath = new Map<string, string>();
      if (cache) {
        if (cacheReadsActive) {
          const { hits, misses } = splitCachedVsFresh(cache, chunkFiles);
          cachedHits = hits;
          for (const miss of misses) contentHashByPath.set(miss.path, miss.contentHash);
          filesToParse = misses.map((m) => ({ path: m.path, content: m.content }));
          applyCachedResults(graph, symbolTable, hits);
          cacheHitFiles += hits.length;
          cacheMissFiles += misses.length;
        } else {
          // Read path off: we'll parse everything, but still hash so the
          // write path can persist results for a future reads-on run.
          for (const f of chunkFiles) {
            contentHashByPath.set(f.path, hashFileContent(f.content));
          }
        }
      }

      // Parse fresh misses (workers or sequential fallback). Request raw
      // per-worker results when cache is active (for the write path).
      ...
      // Merge cached-hit resolution data into chunkWorkerData so downstream
      // import/call/heritage processors see a complete view of this chunk.
      if (cacheReadsActive && chunkWorkerData && cachedHits.length > 0) {
        for (const hit of cachedHits) {
          extendArray(chunkWorkerData.imports, hit.imports);
          extendArray(chunkWorkerData.calls, hit.calls);
          extendArray(chunkWorkerData.assignments, hit.assignments);
          extendArray(chunkWorkerData.heritage, hit.heritage);
          extendArray(chunkWorkerData.routes, hit.routes);
          extendArray(chunkWorkerData.fetchCalls, hit.fetchCalls);
          extendArray(chunkWorkerData.decoratorRoutes, hit.decoratorRoutes);
          extendArray(chunkWorkerData.toolDefs, hit.toolDefs);
          extendArray(chunkWorkerData.ormQueries, hit.ormQueries);
          extendArray(chunkWorkerData.constructorBindings, hit.constructorBindings);
          extendArray(chunkWorkerData.typeEnvBindings, hit.typeEnvBindings);
        }
      }

      // Persist fresh results to cache (one entry per file) for next run
      if (cache && chunkWorkerData?.rawChunkResults && filesToParse.length > 0) {
        const globalNodeFileMap = buildGlobalNodeFileMap(chunkWorkerData.rawChunkResults);
        const perFile = new Map<string, PerFileParseData>();
        for (const rawResult of chunkWorkerData.rawChunkResults) {
          const fileMap = splitResultByFile(rawResult, globalNodeFileMap);
          for (const [filePath, data] of fileMap) {
            const existing = perFile.get(filePath);
            if (!existing) {
              perFile.set(filePath, data);
            } else {
              extendArray(existing.nodes, data.nodes);
              extendArray(existing.symbols, data.symbols);
              extendArray(existing.relationships, data.relationships);
              extendArray(existing.imports, data.imports);
              extendArray(existing.calls, data.calls);
              extendArray(existing.assignments, data.assignments);
              extendArray(existing.heritage, data.heritage);
              extendArray(existing.routes, data.routes);
              extendArray(existing.fetchCalls, data.fetchCalls);
              extendArray(existing.decoratorRoutes, data.decoratorRoutes);
              extendArray(existing.toolDefs, data.toolDefs);
              extendArray(existing.ormQueries, data.ormQueries);
              extendArray(existing.constructorBindings, data.constructorBindings);
              extendArray(existing.typeEnvBindings, data.typeEnvBindings);
            }
          }
        }
        persistParsedFiles(cache, perFile, contentHashByPath);
      }
      ...
      if (chunkWorkerData) {
        ...
        extendArray(deferredWorkerCalls, chunkWorkerData.calls);
        extendArray(deferredWorkerHeritage, chunkWorkerData.heritage);
        extendArray(deferredConstructorBindings, chunkWorkerData.constructorBindings);
        if (chunkWorkerData.assignments?.length) {
          extendArray(deferredAssignments, chunkWorkerData.assignments);
        }

        // Heritage + Routes — calls deferred until all chunks have contributed heritage
        // (complete implementor map for interface dispatch).
        //
        // Phase D step 2b: when cache is active we've just re-injected cached
        // nodes and symbols into the graph. Running heritage+routes concurrently
        // via Promise.all races on ctx state (`enableCache` mutates a shared
        // Map) and has been observed to trigger an intermittent native
        // tree-sitter crash at the Napi boundary — empirically ~20% of warm
        // cache runs. Serializing eliminates the race; measured overhead is
        // negligible because both functions are CPU-light JS loops over
        // pre-extracted data.
        if (cacheActive) {
          await processHeritageFromExtracted(graph, chunkWorkerData.heritage, ctx, (current, total) => {
            onProgress({
              phase: 'parsing',
              percent: Math.round(chunkBasePercent),
          ...
          await processRoutesFromExtracted(graph, chunkWorkerData.routes ?? [], ctx, (current, total) => {
            onProgress({
              phase: 'parsing',
              percent: Math.round(chunkBasePercent),
        ...
        if (chunkWorkerData.typeEnvBindings?.length) {
          extendArray(workerTypeEnvBindings, chunkWorkerData.typeEnvBindings);
        }
        // Collect fetch() calls for Next.js route matching
        if (chunkWorkerData.fetchCalls?.length) {
          extendArray(allFetchCalls, chunkWorkerData.fetchCalls);
        }
        if (chunkWorkerData.routes?.length) {
          extendArray(allExtractedRoutes, chunkWorkerData.routes);
        }
        if (chunkWorkerData.decoratorRoutes?.length) {
          extendArray(allDecoratorRoutes, chunkWorkerData.decoratorRoutes);
        }
        if (chunkWorkerData.toolDefs?.length) {
          extendArray(allToolDefs, chunkWorkerData.toolDefs);
        }
        if (chunkWorkerData.ormQueries?.length) {
          extendArray(allORMQueries, chunkWorkerData.ormQueries);
        }
      } else {
  ...
  for (const chunkPaths of sequentialChunkPaths) {
    ...
    if (chunkFetchCalls.length > 0) {
      extendArray(allFetchCalls, chunkFetchCalls);
    }
...
async function runGraphAnalysisPhases(
  ...
  // Phase D step 9: cache processes by (CALLS edges, memberships, config)
  // hash. If unchanged from a prior compile, skip the BFS entirely.
  const processConfig = { maxProcesses: dynamicMaxProcesses, minSteps: 3 };
  const shouldUseProcessCache =
    repoPath !== undefined && process.env.COTX_FORCE_FULL !== '1';
  const processInputsHash = shouldUseProcessCache
    ? computeProcessInputsHash(graph, communityResult.memberships, processConfig)
    : null;
  let processResult = shouldUseProcessCache && processInputsHash
    ? loadCachedProcesses(repoPath!, processInputsHash)
    : null;
  if (processResult) {
    onProgress({
      phase: 'processes',
      percent: 99,
      message: `Process detection cache hit (${processResult.processes.length} processes)`,
      stats: { filesProcessed: totalFiles, totalFiles, nodesCreated: graph.nodeCount },
    });
  } else {
    ...
    if (shouldUseProcessCache && processInputsHash) {
      try {
        saveCachedProcesses(repoPath!, processInputsHash, processResult);
      } catch {
