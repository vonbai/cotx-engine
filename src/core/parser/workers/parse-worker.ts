  ...
  while (current) {
    if (FUNCTION_NODE_TYPES.has(current.type)) {
      ...
      if (funcName) {
        ...
        const classLookupNode = current.childForFieldName?.('name') ?? current;
        const classInfo = cachedFindEnclosingClassInfo(classLookupNode, filePath);
...
const processBatch = (
  ...
  for (const [language, langFiles] of byLanguage) {
    ...
    if (language === SupportedLanguages.TypeScript) {
      ...
      extendArray(regularFiles, langFiles);
    }
...
function extractLaravelRoutes(tree: Parser.Tree, filePath: string): ExtractedRoute[] {
  ...
  function resolveStack(stack: RouteGroupContext[]): {
    ...
    for (const ctx of stack) {
      extendArray(middleware, ctx.middleware);
...
const processFileGroup = (
  ...
  for (const file of files) {
    ...
    if (provider.isRouteFile?.(file.path)) {
      ...
      extendArray(result.routes, extractedRoutes);
    }
...
const mergeResult = (target: ParseWorkerResult, src: ParseWorkerResult) => {
  extendArray(target.nodes, src.nodes);
  extendArray(target.relationships, src.relationships);
  extendArray(target.symbols, src.symbols);
  extendArray(target.imports, src.imports);
  extendArray(target.calls, src.calls);
  extendArray(target.assignments, src.assignments);
  extendArray(target.heritage, src.heritage);
  extendArray(target.routes, src.routes);
  extendArray(target.fetchCalls, src.fetchCalls);
  extendArray(target.decoratorRoutes, src.decoratorRoutes);
  extendArray(target.toolDefs, src.toolDefs);
  extendArray(target.ormQueries, src.ormQueries);
  extendArray(target.constructorBindings, src.constructorBindings);
  extendArray(target.typeEnvBindings, src.typeEnvBindings);
