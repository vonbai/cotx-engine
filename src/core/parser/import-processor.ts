...
export const processImportsFromExtracted = async (
  ...
  // Phase D step 7 follow-up: sort deterministically so warm runs (cached +
  // fresh extract order) match cold runs (all-fresh order). Stabilizes
  // downstream edge generation order.
  const sortedImports = [...extractedImports].sort((a, b) => {
    const cmp = a.filePath.localeCompare(b.filePath);
    if (cmp !== 0) return cmp;
    return (a.rawImportPath ?? '').localeCompare(b.rawImportPath ?? '');
  });
