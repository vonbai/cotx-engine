...
import { buildClosureSets } from '../../src/compiler/closure-engine.js';
...
describe('buildClosureSets', () => {
  it('expands hard sibling symmetry into must-review closure obligations', () => {
    ...
    const closureSets = buildClosureSets(familyResult, symmetryEdges, []);

    const target = closureSets.find((set) => set.target_unit === 'api:handleCreateUser');
    expect(target?.members.some((member) => member.unit_id === 'api:handleCreateAdmin' && member.level === 'must_review')).toBe(true);
  });
});
